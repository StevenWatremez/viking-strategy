...
Tous les messages de la catégorie choisie sont affichés directement en liste, avec des boutons de copie FR/EN sur chaque carte. Les astuces restent associées à leur sujet. Les fiches d’apprentissage ne sont pas une série à republier intégralement à chaque événement.
...
Les messages, leurs traductions, les astuces `tip` et les compléments `extraMessages` sont regroupés au début de `app.js`. Les styles se trouvent dans `style.css` et le résumé de stratégie dans `index.html`. La copie inclut le titre (`title` en FR, `titleEn` en EN), une ligne vide, puis le texte du message ou de l’astuce. Le titre, les sauts de ligne et le texte doivent tenir dans la limite de 512 caractères du chat.
