...
Page statique adaptée au mobile : stratégie des Vikings et messages traduits en français, anglais, espagnol, portugais du Brésil et arabe, avec traduction intégrale du site et copie en un clic.
...
Le sélecteur de langue situé dans la barre supérieure (`FR` / `EN` / `ES` / `PT-BR` / `العربية`) traduit l'ensemble du site et des messages, et adapte la direction d'écriture (LTR / RTL). Sur chaque carte de message, les boutons de copie s'adaptent selon la langue choisie :
...
Les messages et leurs traductions (`fr`, `en`, `es`, `pt-BR`, `ar` avec `title`, `titleEn`, `titleEs`, `titlePtBr`, `titleAr`) ainsi que le dictionnaire d'interface (`uiTranslations`) sont regroupés dans `app.js`. Les styles se trouvent dans `style.css` et le balisage initial dans `index.html`. La copie inclut le titre, une ligne vide, puis le texte du message. Le titre, les sauts de ligne et le texte doivent tenir dans la limite de 512 caractères du chat.
