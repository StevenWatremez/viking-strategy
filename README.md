...
Les messages et leurs traductions (`fr`, `en`, `es`, `pt-BR`, `ar`, `pl`, `tr` avec `title`, `titleEn`, `titleEs`, `titlePtBr`, `titleAr`, `titlePl`, `titleTr`) ainsi que le dictionnaire d'interface (`uiTranslations`) sont regroupés dans `app.js`. Les styles se trouvent dans `style.css` et le balisage initial dans `index.html`. La copie inclut le titre, une ligne vide, puis le texte du message. Le titre, les sauts de ligne et le texte doivent tenir dans la limite de 512 caractères du chat.
